...
const BinaryMonitor: React.FC<BinaryMonitorProps> = ({ packets }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-black border-t-2 border-green-900 font-mono text-xs">
      <div className="flex justify-between items-center px-2 py-1 bg-green-900/20 border-b border-green-900/50">
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className="flex items-center gap-1.5 px-2 py-0.5 rounded-sm bg-green-900/30 text-[10px] text-green-400 hover:text-green-200 hover:bg-green-900/50 transition-all border border-green-900/50"
            title="Toggle Payload Encoding"
          >
             {viewMode === 'HEX' ? <FileCode size={10} /> : <FileText size={10} />}
             <span>{viewMode}</span>
          </button>
      ...
      >
        {packets.length === 0 && (
          <div className="text-gray-600 italic">Waiting for bus traffic...</div>
        )}
        {packets.map((packet, idx) => {
          ...
          return (
            <div key={`${packet.timestamp}-${idx}`} className="flex gap-4 opacity-80 hover:opacity-100 transition-opacity">
              <span className="text-gray-500 w-24 shrink-0">
                {packet.timestamp.toFixed(4).slice(-8)}
              </span>
              <span className={`w-32 shrink-0 ${isHigh || isError ? 'text-red-500 font-bold' : 'text-green-600'}`}>
              ...
              <span className="text-gray-400 break-all flex-1">
                ...
                <span className="text-gray-600 ml-2 text-[10px] select-none">CRC:{Math.floor(Math.random() * 255).toString(16).toUpperCase()}</span>
              </span>
            </div>
          );
        })}
